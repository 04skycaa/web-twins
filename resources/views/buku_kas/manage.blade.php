    ...
    <div id="view-hutang" class="view-section {{ $active_tab === 'hutang' ? 'active' : '' }}">
        <div class="main-content-box mobile-pb">
                    ...
                    <tbody>
                        ...
                        @php
                            $debtData = [
                                ...
                                'outlet_nama' => $h->outlet->nama ?? null
                            ];
            ...
            <div class="buku-kas-card-grid" style="display: none; flex-direction: column; gap: 12px; margin-top: 8px;">
                @forelse($hutang as $h)
                @php
                    $debtData = ['uuid' => $h->uuid, 'tipe' => $h->tipe, 'nominal' => $h->nominal, 'sisa' => $h->sisa, 'jatuh_tempo' => $h->jatuh_tempo, 'transaction_id' => $h->transaction_id ?? null, 'payment_order_id' => $h->payment_order_id ?? null, 'outlet_nama' => $h->outlet->nama ?? null];
                    $contactData = ['nama' => $h->contact->nama ?? null];
    ...
    <div id="view-piutang" class="view-section {{ $active_tab === 'piutang' ? 'active' : '' }}">
        <div class="main-content-box mobile-pb">
                    ...
                    <tbody>
                        ...
                        @php
                            $debtData = [
                                ...
                                'outlet_nama' => $p->outlet->nama ?? null
                            ];
            ...
            <div class="buku-kas-card-grid" style="display: none; flex-direction: column; gap: 12px; margin-top: 8px;">
                @forelse($piutang as $p)
                @php
                    $debtData = ['uuid' => $p->uuid, 'tipe' => $p->tipe, 'nominal' => $p->nominal, 'sisa' => $p->sisa, 'jatuh_tempo' => $p->jatuh_tempo, 'transaction_id' => $p->transaction_id ?? null, 'payment_order_id' => $p->payment_order_id ?? null, 'outlet_nama' => $p->outlet->nama ?? null];
                    $contactData = ['nama' => $p->contact->nama ?? null];
